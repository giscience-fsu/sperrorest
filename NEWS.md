...
Features: 
  * error handling during model fitting: If a model does not converge for some folds, results of this fold are set to `NA` and a message is printed to the console. `sperrorest()` will continue normally and uses the successful folds to calculate the repetition error. Non-convergence during model fit happens frequenly for some models, e.g. `maxnet()` or `gamm()`.
