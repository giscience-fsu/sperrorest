...
res.lda.sp.par <- parsperrorest(fo, data = maipo, coords = c("utmx","utmy"), 
                               ...
                               pred.args = list(fac = "field"),
                               smp.fun = partition.factor.cv,
                               smp.args = list(fac = "field", repetition = 1:3, nfold = 5),
                               par.args = list(par.units = 2, par.mode = 1, 
                                               lb = FALSE, high = T),
                               error.rep = TRUE, error.fold = T,
                               benchmark = TRUE)
...
```{r}
res.lda.sp.par <- parsperrorest.old(fo, data = maipo, coords = c("utmx","utmy"), 
                               model.fun = lda,
                               pred.fun = lda.predfun, 
                               pred.args = list(fac = "field"),
                               smp.fun = partition.cv,
...
```{r}
res.bagg.sp.par3 <- parsperrorest.old(fo, data = maipo, coords = c("utmx","utmy"), 
                                   model.fun = bagging,
                                   pred.args = list(fac = "field"),
                                   smp.fun = partition.factor.cv,
                                   smp.args = list(fac = "field", repetition = 1:5, nfold = 5),
                                   par.args = list(par.units = 2, par.mode = 2,
                                                   lb = FALSE, high = T),
                                   error.rep = TRUE, error.fold = TRUE,
                                   benchmark = TRUE)
