...
Ich schlage eine eigene Parallelvignette vor (sofern dies überhaupt nötig ist). Die basics werden ja auch in den Beispielen voj `parsperrorest()` gezeigt. 
...
* Ich würde zwei Modi Vorschlagen: Der Erste (`par.mode = 1`) ist der `pbapply` Ansatz, welcher automatisch zwischen `mclapply` und `parApply` unterscheidet nach Plattform Typ. Der Zweite (`par.mode = 2`) ist der `foreach` Ansatz, der zwar langsamer läuft, aber manchmal stabiler.
...
Sind im `NEWS.md` file zusammengefasst. Dazu bitte das Paket von Github installieren!
