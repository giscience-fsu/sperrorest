...
tile.neighbors = function(nm, tileset, iterate = 0, diagonal = FALSE)
{
    ...
    if (!is.null(tileset)) {
        # If neighbors are not in 'tileset' (i.e. have been removed e.g. 
        # because they were too small)
        # then continue search up to 'iterate' times/steps:
        if ( !any(nbr %in% tileset) & (iterate > 0) ) {
            nbrs = c()
            for (a.nbr in nbr)
                nbrs = c(nbrs, tile.neighbors(nm = a.nbr, tileset = tileset, 
                                              iterate = iterate - 1))
            nbr = nbrs
            nbr = nbr[ nbr != nm ]
        }
        ...
        # i.e. have
        # been removed previously (presumably because they were too small, or 
        # out of range):
...
as.represampling = function(object, ...)
    if (inherits(object,"represampling")) object else 
...
summary.represampling = function(object, ...)
{
  lapply(object, function(x) as.data.frame( t(sapply(x, function(y) 
    data.frame(n.train = length(y$train), n.test = length(y$test)))) ) )
}
