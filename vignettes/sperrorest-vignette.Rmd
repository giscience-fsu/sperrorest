...
```{r}
predictors <- colnames(d)[5:ncol(d)]
# Construct a formula:
...
```{r}
partialPlot(fit, d, x.var = "ndvi01", which.class = "crop1")
partialPlot(fit, d, x.var = "ndvi01", which.class = "crop3")
```
...
```{r}
res.lda.nsp <- res <- sperrorest(fo, data = d, coords = c("utmx","utmy"), 
                                 model.fun = lda,
...
round(summary(res.lda.nsp$pooled.error), 3)
```
...
```{r}
res.lda.sp <- sperrorest(fo, data = d, coords = c("utmx","utmy"), 
                         model.fun = lda,
                         pred.fun = lda.predfun, 
                         pred.args = list(fac = "field"),
                         ...
                         err.pooled = TRUE, err.unpooled = FALSE)
round(summary(res.lda.sp$pooled.error), 3)
```
...
```{r}
res.rf.sp <- sperrorest(fo, data = d, coords = c("utmx","utmy"), 
...
```{r}
res.rf.sp.par <- parsperrorest(fo, data = d, coords = c("utmx","utmy"), 
                               model.fun = randomForest,
                               pred.fun = rf.predfun, 
                               pred.args = list(fac = "field"),
                               smp.fun = partition.factor.cv,
...
Läuft (partition.factor.cv sorgt dafür dass folds nicht geprintet werden) 
```{r}
